-- Update 2FA secret with the new one generated by the setup page
-- Run this in your Supabase SQL Editor: https://qqcoooscyzhyzmrcvsxi.supabase.co

-- Update the admin account with the new 2FA secret
UPDATE admin_access 
SET 
  two_factor_secret = 'EVUXV76SPLMOHG6ZN3M4B5',  -- New secret from QR code
  two_factor_enabled = true,
  updated_at = NOW()
WHERE user_id = (SELECT id FROM users WHERE email = 'wasando.tsuyukusa@gmail.com');

-- Verify the update
SELECT '2FA secret updated successfully!' as status;
